...
% constant density. When the antenna is short, '%1.1f' should be used in
...
T_e = 10.^(-2:-1);
V2 = zeros(length(f), length(T_e));
Z = V2;
S = cell(1, length(n_e));
for i = 1:length(T_e);
    [V2(:,i), Z(:,i)] = eo.qtnmod(n_e, T_e(i), f);
    S(i) = {['10$$^{' num2str(log10(T_e(i))) '}$$ eV ($$L/\lambda_{D_m}$$ = ' num2str(6/eo.debye(n_e, T_e(i)), '%1.1f') ')']};
end
